...
// From https://github.com/gnosis/1155-to-20/blob/master/contracts/Wrapped1155Factory.sol
...
contract Wrapped1155Factory is ERC1155Receiver_06Port {
    ...
    function onERC1155BatchReceived(
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            uint256 first = i.mul(65);
            uint256 next = first.add(65);
    ...
    // TODO: Test that this produces the same results are the 0.6 version.
    ...
    event Wrapped1155Creation(IERC1155 indexed multiToken, uint256 indexed tokenId, Wrapped1155 indexed wrappedToken);
        ...
        returns (Wrapped1155)
    {
        ...
        address wrapped1155Address = address(
            uint160(uint256(keccak256(abi.encodePacked(uint8(0xff), this, uint256(1155), keccak256(deployBytecode)))))
        );

        // TODO: test this behaves the same as !Address.isContract()
        if (wrapped1155Address.code.length == 0) {
            address addr;
            assembly {
            ...
            emit Wrapped1155Creation(multiToken, tokenId, Wrapped1155(wrapped1155Address));
        }
