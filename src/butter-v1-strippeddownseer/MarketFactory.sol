...
contract MarketFactory {
    ...
    event NewMarket(address indexed market, string marketName, bytes32 conditionId, bytes32 questionId);
    ...
    function createCategoricalMarket(CreateMarketParams calldata params) external returns (address) {
        ...
        encodedQuestions[0] =
            encodeRealityQuestionWithOutcomes(params.marketName, ["UP", "DOWN"], params.category, params.lang);
    ...
    function createMarket(
        ...
        instance.initialize(
            marketName, params.lowerBound, params.upperBound, conditionalTokensParams, realityParams, realityProxy
        );

        emit NewMarket(
            address(instance), marketName, conditionalTokensParams.conditionId, conditionalTokensParams.questionId
        );
    ...
    function createNewMarketParams(CreateMarketParams memory params, InternalMarketConfig memory config)
        internal
        returns (Market.ConditionalTokensParams memory, Market.RealityParams memory)
    {
        bytes32 parentCollectionId = bytes32(0); // There won't be any parent collection
        ...
        bytes32 questionId =
            keccak256(abi.encode(questionsIds, 2, config.templateId, params.lowerBound, params.upperBound));
    ...
    function askRealityQuestion(string memory encodedQuestion, uint256 templateId, uint32 openingTime, uint256 minBond)
        internal
        returns (bytes32)
    {
