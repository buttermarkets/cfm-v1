...
contract CreateDecisionMarketBase is DeployCoreContractsBase {
    ...
    FlatCFM cfm2;
    ConditionalScalarMarket conditionalMarketA2;
    ConditionalScalarMarket conditionalMarketB2;
    ConditionalScalarMarket conditionalMarketC2;
    ...
    function _recordConditionIdAndScalarMarkets2() internal {
        ...
        for (uint256 i = 0; i < logs.length; i++) {
            ...
            if (
                ...
                address csmAddr = address(uint160(uint256(logs[i].topics[2])));
                if (found == 0) {
                    conditionalMarketA2 = ConditionalScalarMarket(csmAddr);
                } else if (found == 1) {
                    conditionalMarketB2 = ConditionalScalarMarket(csmAddr);
                } else if (found == 2) {
                    conditionalMarketC2 = ConditionalScalarMarket(csmAddr);
                }
                found++;
            }
        }
